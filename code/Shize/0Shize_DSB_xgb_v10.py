...
for z in range(1,6):
    ...
    if(z==0):
        ...
        sys.stderr = open('pred/Shize_DSB_xgb_v10_LBfullTrain'+'.txt', 'w')
        ...
        submission.to_csv('pred/Shize_DSB_xgb_v10_test(FullTrain).csv', index=False)
    ...
    print('Start model training:')
    oldstderr = sys.stderr 
    sys.stderr = open('pred/Shize_DSB_xgb_v10_xgb_fold'+str(z)+'.txt', 'w')
    sys.stderr = flushfile(sys.stderr) 
    #clf2 = xgb.train(param, xg_train, num_round, evals=watchlist, early_stopping_rounds=5, verbose_eval=True)
    ...
    cv_pred['cancer'] = pred
    #cv_pred.to_csv('pred/Shize_DSB_xgb_v10_cv'+str(z)+'.csv', index=False)
    print(cv_pred.shape)
    
    print("Done save cv predictions.")
    
    print('Shize_DSB_xgb_v10_xgb logloss CV score,fold '+str(z)+':')
    ...
    if(z==5):
        ...
        full_cv_pred.to_csv('pred/Shize_DSB_xgb_v10_cv_pred_full.csv', index=False)    
    ...
    if(z==5):
        ...
        submission.sort(['id'], ascending=[True], inplace=True)
        submission.to_csv('pred/Shize_DSB_xgb_v10_xgb_test_5foldAverage.csv', index=False)
