...
for run in range(0,num_run):
    ...
    for z in range(0,6):
        ...
        np.random.seed(88)
        rseed=8
        clf2 = ensemble.ExtraTreesClassifier(n_jobs=30,criterion='gini', n_estimators=800, max_depth=20, min_samples_leaf=6 , random_state=rseed+run)  #6: 0.443315, #8:0.44368
    
        if(z==0):
            print('Start model training:')
            #num_round=78 #           
            oldstderr = sys.stderr 
            sys.stderr = open('pred/Shize_DSB_etr_v21_10run_LBfullTrain'+'.txt', 'w')
            ...
            submission.to_csv('pred/Shize_DSB_etr_v21_10run_test(FullTrain).csv', index=False)
        ...
        print('Start model training:')
        oldstderr = sys.stderr 
        sys.stderr = open('pred/Shize_DSB_etr_v21_10run_fold'+str(z)+'.txt', 'w')
        sys.stderr = flushfile(sys.stderr) 
        clf2.fit(train_x, train_y)
        sys.stderr = oldstderr
        print('End model training:')
        ...
        print('Shize_DSB_etr_v21_10run logloss CV score,fold '+str(z)+':')
        ...
        if(z==5):
            ...
            full_cv_pred.to_csv('pred/Shize_DSB_etr_v21_10run_cv_pred_full.csv', index=False)    
        ...
        if(z==5):
            ...
            submission.sort(['id'], ascending=[True], inplace=True)
            submission.to_csv('pred/Shize_DSB_etr_v21_10run_test_5foldAverage.csv', index=False)
    ...
    if(run==num_run-1):
        ...
        sub.to_csv('pred_10run/Shize_DSB_etr_v21_10run_averaged.csv', index=False)
        
        sub_cv['cancer']=sub_cv['cancer']/(1.0*num_run)
        sub_cv.to_csv('pred_10run/Shize_DSB_etr_v21_10run_averaged_cvpred.csv', index=False)
