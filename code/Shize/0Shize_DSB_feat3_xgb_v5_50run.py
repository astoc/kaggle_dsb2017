...
for run in range(0,num_run):
    ...
    for z in range(0,6):
        ...
        if(z==0):
            ...
            sys.stderr = open('pred/Shize_DSB_feat3_xgb_v5_50run_LBfullTrain'+'.txt', 'w')
            ...
            submission.to_csv('pred/Shize_DSB_feat3_xgb_v5_50run_test(FullTrain).csv', index=False)
        ...
        print('Start model training:')
        oldstderr = sys.stderr 
        sys.stderr = open('pred/Shize_DSB_feat3_xgb_v5_50run_xgb_fold'+str(z)+'.txt', 'w')
        sys.stderr = flushfile(sys.stderr) 
        #clf2 = xgb.train(param, xg_train, num_round, evals=watchlist, early_stopping_rounds=5, verbose_eval=True)
        ...
        cv_pred['cancer'] = pred
        #cv_pred.to_csv('pred/Shize_DSB_feat3_xgb_v5_50run_cv'+str(z)+'.csv', index=False)
        print(cv_pred.shape)
        
        print("Done save cv predictions.")
        
        print('Shize_DSB_feat3_xgb_v5_50run_xgb logloss CV score,fold '+str(z)+':')
        ...
        if(z==5):
            ...
            full_cv_pred.to_csv('pred/Shize_DSB_feat3_xgb_v5_50run_cv_pred_full.csv', index=False)    
        ...
        if(z==5):
            ...
            submission.sort(['id'], ascending=[True], inplace=True)
            submission.to_csv('pred/Shize_DSB_feat3_xgb_v5_50run_xgb_test_5foldAverage.csv', index=False)
    ...
    if(run==num_run-1):
        ...
        sub.to_csv('pred_50run/Shize_DSB_feat3_xgb_v5_50run_averaged.csv', index=False)
        
        sub_cv['cancer']=sub_cv['cancer']/(1.0*num_run)
        sub_cv.to_csv('pred_50run/Shize_DSB_feat3_xgb_v5_50run_averaged_cvpred.csv', index=False)
