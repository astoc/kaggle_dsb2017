...
#df.to_csv('feats_keras_0311_sorted_id.csv', index=False)
print(df.iloc[0:3,0:5])
...
#df2.to_csv('feats_keras_0311_sorted_id.csv', index=False)
print(df2.iloc[0:3,0:5])
...
for run in range(0,num_run):
    ...
    for z in range(0,6):
        ...
        param['eta'] = 0.05                #0.05  #0.01 
        param['eval_metric'] = "logloss"
        param['subsample'] = 0.5           #0.6 #0.83 
        param['colsample_bytree'] = 0.3    #3 #0.8 
        param['max_depth'] = 3             #3  #20 
        param['seed'] = 8888 +run*100            #1 #8 #88  #8888
        param['nthread'] = 30              #2 #4 #7 
        param['min_child_weight']=1
        #param['base_score'] = 0.2879
        param['gamma']=0.03
        param['lambda']=0.77 #0.77
        param['scale_pos_weight']=2.1 #2.1
        ...
        #clf2 = xgb.train(param, xg_train, num_round, evals=watchlist)    
        #clf2 = xgb.train(param, xg_train, num_round, evals=watchlist, early_stopping_rounds=5, verbose_eval=True)
    
        if(z==0):
            print('Start model training:')
            num_round=110 #           
            oldstderr = sys.stderr 
            sys.stderr = open('pred/Shize_DSB_allFea_xgb_v1_50run_LBfullTrain'+'.txt', 'w')
            ...
            submission.to_csv('pred/Shize_DSB_allFea_xgb_v1_50run_test(FullTrain).csv', index=False)
            
            sub_FullTrain=submission.copy()
        ...
        print('Start model training:')
        oldstderr = sys.stderr 
        sys.stderr = open('pred/Shize_DSB_allFea_xgb_v1_50run_xgb_fold'+str(z)+'.txt', 'w')
        sys.stderr = flushfile(sys.stderr) 
        #clf2 = xgb.train(param, xg_train, num_round, evals=watchlist, early_stopping_rounds=5, verbose_eval=True)
        ...
        cv_pred['cancer'] = pred
        #cv_pred.to_csv('pred/Shize_DSB_allFea_xgb_v1_50run_cv'+str(z)+'.csv', index=False)
        print(cv_pred.shape)
        
        print("Done save cv predictions.")
        
        print('Shize_DSB_allFea_xgb_v1_50run_xgb logloss CV score,fold '+str(z)+':')
        ...
        if(z==5):
            ...
            full_cv_pred.to_csv('pred/Shize_DSB_allFea_xgb_v1_50run_cv_pred_full.csv', index=False)    
        ...
        if(z==5):
            ...
            submission.sort(['id'], ascending=[True], inplace=True)
            submission.to_csv('pred/Shize_DSB_allFea_xgb_v1_50run_xgb_test_5foldAverage.csv', index=False)
    ...
    if (run==0):
        sub=submission
        sub['cancer']=submission['cancer']*0.7+sub_FullTrain['cancer']*0.3
        sub_cv=full_cv_pred
        
    if(run>0):
        sub['cancer']=sub['cancer']+submission['cancer']*0.7+sub_FullTrain['cancer']*0.3
        sub_cv['cancer']=sub_cv['cancer']+full_cv_pred['cancer']
        
    if(run==num_run-1):
        sub['cancer']=sub['cancer']/(1.0*num_run)
        sub.to_csv('pred_50run/Shize_DSB_allFea_xgb_v1_50run_averaged.csv', index=False)
        
        sub_cv['cancer']=sub_cv['cancer']/(1.0*num_run)
        sub_cv.to_csv('pred_50run/Shize_DSB_allFea_xgb_v1_50run_averaged_cvpred.csv', index=False)
